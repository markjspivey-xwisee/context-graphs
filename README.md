...
- **Per-surface agents, auto-detected.** Each client (Claude Code, Claude Desktop, Claude Mobile via OAuth, ChatGPT, OpenAI Codex, Cursor, Windsurf, Cline, …) registers as its own `cg:AuthorizedAgent` with its own `did:web` identity and X25519 keypair. The relay maps the OAuth client's DCR-registered `client_name` to a surface slug automatically; unknown clients get their slugified name, never a silent `claude-*` fallback. Envelope recipient sets grow as you add surfaces; each surface decrypts with its own key; revocation is per-surface.
...
- **Decentralized auth — no passwords, no user-claimable identifiers.** Sign-in via SIWE (ERC-4361), WebAuthn passkeys, or `did:key` Ed25519 signatures. The userId is *derived* from your first credential (`u-pk-<hash>` / `u-eth-<addr>` / `u-did-<hash>`) — never a string you type, so an attacker who learns your display alias can't bind their credential to your account. Legacy seeded userIds are gated behind single-use `BOOTSTRAP_INVITES`. Identity server is a stateless DID resolver + signature verifier; user auth state lives in `<pod>/auth-methods.jsonld` (JSON-LD). See [`deploy/identity/AUTH-ARCHITECTURE.md`](deploy/identity/AUTH-ARCHITECTURE.md).
