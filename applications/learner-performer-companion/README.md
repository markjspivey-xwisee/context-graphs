...
4. Adds context: "You completed this module on 2026-04-15 (xAPI ref) and earned Open Badge 3.0 issued by Acme Training on 2025-Q3"
...
| **Tier 5** — [`tests/tier5-vc-roundtrip.test.ts`](tests/tier5-vc-roundtrip.test.ts) issues a real OB 3.0-shaped credential signed by Acme Training's ECDSA wallet, verifies it from a third-party verifier wallet, detects tampered turtle (single-byte change → rejected), detects forgery (different signer → recovered address mismatch), and proves portability (credential signed by Acme Training still verifies after Mark switches employers, no Acme Training API call needed) | |
