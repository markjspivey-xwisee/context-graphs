 ...
 *   1. Issue an OB 3.0-shaped credential descriptor signed by Acme Training's wallet
 ...
 *   5. After Mark moves employers, Acme Training's signature still verifies
 *      (the wallet is the issuer; the verifier never needs Acme Training's
...
describe('Tier 5 — Verifiable Credential cryptographic roundtrip', () => {
  ...
  it('forgery: a competitor signing the same content with their own key fails issuer match', async () => {
    ...
    // Competitor signs Acme Training's credential content with their own key
    ...
    // But the recovered address is the COMPETITOR's, not Acme Training's.
  ...
  it('portability: credential signed by Acme Training still verifies after Mark switches employers', async () => {
    const acme = importWallet(ACME_TRAINING_KEY, 'agent', 'acme-training');
    const newEmployer = importWallet(NEW_EMPLOYER_KEY, 'agent', 'new-employer');

    // Mark earned the credential at Acme Training on 2025-09-15
    const acmeDid = `did:web:acme-training.example#${acme.address.toLowerCase()}` as IRI;
    const credential = buildOB3CredentialDescriptor(
      'did:web:mark.example' as IRI,
      acmeDid,
    ...
    // Acme Training's credential without Acme Training's API being available, because
    ...
    expect(result.valid).toBe(true);
    expect(result.recoveredAddress?.toLowerCase()).toBe(acme.address.toLowerCase());
    // → New employer can confirm Acme Training issued this without contacting Acme Training
  });
